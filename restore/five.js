...
import fs from "fs"
import { Instrument, kline_5m } from "../mongo/schema.js"
...
if (instruments.length) {
	...
	let x = 1
	fs.createReadStream(`restore/five/${file}.csv`)
		.pipe(csv.parse({ headers: true }))
		.on("error", error => console.error(error))
		.on("data", row => {
			console.log(
				`received candle #${x}: ${row.symbol} - ${row.startISO}`
			)
			data.push(row)
			x += 1
		})
		.on("end", async () => {
			...
			for await (let instrument of instruments) {
				...
				if (candles.length) {
					...
					console.log(
						`${instrument.symbol}: ${candles.length} candles`
					)
				}
			}
			mongoose.disconnect()
